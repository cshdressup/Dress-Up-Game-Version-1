...
let skinNames = ["Dark Skin Tone 1", "Dark Skin Tone 2", "Dark Skin Tone 3", "Medium Skin Tone 1", "Medium Skin Tone 2", "Light Skin Tone 1", "Light Skin Tone 2"];
let faceNames = ["Smiley Face", "Frowny Face", "Silly Face", "Neutral Face", "Confused Face", "Dizzy Face"];
let hairNames = ["Short Straight Hair", "Short Straight Hair with Side Part", "Short Wavy Hair", "Short Wavy Hair with Side Bangs", "Short Curly Hair", "Curly Bun", "Medium-length Straight Hair with Bangs", "Long Wavy Hair", "Long Straight Hair", "Long Curly Hair", "Long Straight Ponytail", "Long Box Braids"];
let hairColorNames = ["Black", "Dark Brown", "Light Brown", "Blonde", "Red", "Grey", "Purple", "Blue"];
let shirtNames = ["Blue Tee Shirt", "Black Tee Shirt with Bow", "Green Striped Sweater", "Red Tank Top", "Orange Striped Tee Shirt", "Purple Tank Top", "Yellow Tee Shirt with Heart", "Purple Tee Shirt", "Pink Tee Shirt with Bow", "Yellow Striped Sweater", "Blue Tank Top", "Grey Striped Tee Shirt", "Green Tank Top", "Pink Tee Shirt with Heart"];
let pantsNames = ["Black Pants", "Blue Jeans", "White Pants", "Pink Pants", "Brown Pants", "Black Skirt", "Blue Denim Skirt", "White Skirt", "Pink Skirt", "Brown Skirt"];
let shoeNames = ["Red Sneakers", "Black Boots", "Pink Sneakers", "Black Dress Shoes", "Brown Cowboy Boots", "Blue Sneakers", "Black Flats"];
let hatNames = ["No Hat", "Sunglasses", "Round Glasses", "Black Baseball Cap", "Gold Tiara", "Black Top Hat", "Pink Bow", "Blue Headband", "Black Beret", "Red Beanie"];

let hairColors = ["#221006", "#6a3a17", "#c47c31", "#ffd699", "#d07623", "#b4b4b4", "#8f1eae", "#5aa4e8"];
let pantsColors = ["#494949", "#7c95bd", "#f1f1f1", "#e7a39f", "#715540"]; 
...
let selectedShirt = 11, selectedPantsImg = 0, selectedPantsCol = 1, selectedShoes = 0, selectedHat = -1;         
...
function preload() {
  for (let i = 0; i <= 6; i++) skins.push(loadImage('assets/skin' + i + '.png'));
  for (let i = 0; i <= 5; i++) faces.push(loadImage('assets/face' + i + '.png'));
  for (let i = 0; i <= 11; i++) hairs.push(loadImage('assets/hair' + i + '.png'));
  for (let i = 0; i <= 13; i++) shirts.push(loadImage('assets/shirt' + i + '.png'));
  for (let i = 0; i <= 1; i++) pants.push(loadImage('assets/pants' + i + '.png'));
  for (let i = 0; i <= 6; i++) shoes.push(loadImage('assets/shoes' + i + '.png'));
  for (let i = 0; i <= 8; i++) hats.push(loadImage('assets/hat' + i + '.png'));
}
...
function drawAvatar() {
  ...
  const renderImg = (img, cat, tintCol) => {
    if (tintCol) tint(tintCol); else noTint();
  ...
  renderImg(currentCategory === "Skin Tones" && tempIndex !== -1 ? skins[tempIndex] : skins[selectedSkin], "Skin Tones");
  ...
  if (currentCategory === "Pants and Skirts" && tempIndex !== -1) {
    pIdx = tempIndex > 4 ? 1 : 0; cIdx = tempIndex % 5;
  } else {
...
function drawFinal() {
  ...
  image(faces[selectedFace], 0, 0, IMG_W, IMG_H);
  image(shoes[selectedShoes], 0, 0, IMG_W, IMG_H);
  push(); tint(pantsColors[selectedPantsCol]); image(pants[selectedPantsImg], 0, 0, IMG_W, IMG_H); pop();
  image(shirts[selectedShirt], 0, 0, IMG_W, IMG_H);
  push(); tint(hairColors[selectedHairColor]); image(hairs[selectedHair], 0, 0, IMG_W, IMG_H); pop();
  if (selectedHat >= 0) image(hats[selectedHat], 0, 0, IMG_W, IMG_H);
  pop();
...
function selectOption() {
  ...
  else if (currentCategory === "Pants and Skirts") { selectedPantsImg = selection > 4 ? 1 : 0; selectedPantsCol = selection % 5; currentCategory = "Shoes"; }
...
function resetGame() {
  ...
  selectedShirt = 11; selectedPantsImg = 0; selectedPantsCol = 1; selectedShoes = 0; selectedHat = -1;
  confetti = []; 
}
